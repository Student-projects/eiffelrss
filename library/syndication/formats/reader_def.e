...
feature {NONE} -- Implementation
	...
	read_or_default_element (a_element: detachable XML_ELEMENT; default_value: STRING): STRING
		...
		do
			...
			if
				a_element /= Void and then
				attached a_element.text as l_text and then
				not l_text.is_empty
			then
				Result := l_text
			else
	...
	read_or_default_attribute (a_attribute: detachable XML_ATTRIBUTE; default_value: STRING): STRING
		...
		do
			Result := default_value
			if
				a_attribute /= Void and then
				not a_attribute.value.is_empty
			then
	...
	valid_element_text (an_element: detachable XML_ELEMENT; a_name: STRING): BOOLEAN
		...
		do
			if
				an_element /= Void and then
				attached an_element.element_by_name (a_name) as l_elt and then
				attached l_elt.text as l_elt_text and then
				not l_elt_text.is_empty
			then
				Result := True
			end
		end
